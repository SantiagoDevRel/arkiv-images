...
5. Explore **How this image was retrieved**. Click **Image entity**, **Manifest** or **Chunks** to inspect real keys, typed attributes, payload sizes and hex previews. For chunks, select an individual part to see its zero-based byte range. **See the inspection query** shows the read-only SDK query with actual keys and snapshot block. The manifest's JSON metadata and the image's binary payload are explicitly distinguished.
